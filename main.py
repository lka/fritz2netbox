...
def main():
    ...
    hosts = FritzBox.get_active_hosts(None, hosts, ignore_list)
    ...
    for host in hosts_v4:
        hostname = host["name"].casefold()
        found_in_nb = nb.search_hosts_with_dns_name(nb_v4_hosts, hostname)
                ...
                if nb.has_interface(found_in_nb[0]):
                    ...
                    if resp.status_code == 200:
                        ...
                        if interface["mac_address"] != host["mac"]:
                            ...
                            resp = nb.get_mac_adresses()
                            if resp.status_code != 200:
                                continue
                            macList = json.loads(resp.text)["results"]
                            macID = nb.search_macList_with_address(
                                macList, host["mac"]
                            )[0]["id"]
